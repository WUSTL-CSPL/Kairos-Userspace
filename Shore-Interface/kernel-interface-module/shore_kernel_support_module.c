 ...
 * Check dmesg for kernel module loading information, create a device file using mknod following the instructions in the kernel module loading information (sudo privileges required).
 * 
 * Writing to the device file to set the Shore_id, priority_level, and operation_mode (sudo privileges required):
 * 
 * Set the priority level or remove the shore enforced program:
 * echo "shore_id,priority_level,operation_mode" > /dev/shore_kernel_module (1 comma)
 * 
 ...
 * 1 is reserved to the system, does not prioritize nor recover to normal priority. 
 * DO NOT USE -1.
 * 
 * @operation_mode: The operation mode to perform. 0 is to set the priority level for the shore ID. 1 is to remove the shore ID from the Shore enforced program list.
 * 
...
extern void shore_add_cpu_task_to_dfa_list(int new_shore_id, int new_shore_priority);
...
extern void shore_add_network_task_to_dfa_list(int shore_net_id, int shore_net_priority);
extern int shore_set_network_task_priority(int shore_net_id, int shore_net_priority);
...
#define SUCCESS 0
#define DEVICE_NAME "shore_kernel_module" /* Dev name as it appears in /proc/devices */
...
static ssize_t device_write(struct file *filp, const char *buff, size_t len,
                            loff_t *off) {
    /**
     * This function receives a message from user space and extracts the shore ID and priority level.
     *
    ...
    int shore_id, shore_priority_level, operation_mode; // Variables to store the shore ID, priority level, and return value
    char *separator1, *separator2; // Pointer to a character separator

    memset(received_msg, 0, 1024); // Clear the received message buffer
    count = copy_from_user(received_msg, buff, len); // Copy the message from user space to the received message buffer

    separator1 = strchr(received_msg, ','); // Find the separator in the message
    ...
    separator2 = strchr(separator1 + 1, ','); // Find the second separator in the message

    if (separator2) {
        // Two commas found
        *separator1 = '\0'; // Temporarily end the string at the first comma for parsing
        *separator2 = '\0'; // Temporarily end the string at the second comma for parsing

        shore_id = simple_strtoul(received_msg, NULL, 10); // Extract the shore ID from the message
        shore_priority_level = simple_strtoul(separator1 + 1, NULL, 10); // Extract the priority level from the message
        operation_mode = simple_strtoul(separator2 + 1, NULL, 10); // Extract the operation mode from the message

        // *separator = ','; // Restore the second comma in case the buffer is needed again

        // simple_strtol returns long, so make sure the values are within int range
        if (shore_id > INT_MAX || shore_id < INT_MIN || shore_priority_level > INT_MAX || shore_priority_level < INT_MIN || operation_mode > INT_MAX || operation_mode < INT_MIN) {
        ...
        printk("Invalid usage, please use the following format: shore_id,priority_level,operation_mode\n");
        return -EINVAL; // Invalid number of commas
    }

    // *separator1 = '\0'; // Temporarily end the string at the comma for parsing

    // shore_id = simple_strtoul(received_msg, NULL, 10); // Extract the shore ID from the message
    // shore_priority_level = simple_strtoul(separator1 + 1, NULL, 10); // Extract the priority level from the message

    // // *separator = ','; // Restore the comma in case the buffer is needed again

    // // simple_strtol returns long, so make sure the values are within int range
    // if (shore_id > INT_MAX || shore_id < INT_MIN || shore_priority_level > INT_MAX || shore_priority_level < INT_MIN) {
    //     return -ERANGE; // Value out of int range
    // }
    // shore_add_cpu_task_to_dfa_list(shore_id, shore_priority_level);
    // shore_add_network_task_to_dfa_list(shore_id, shore_priority_level);
