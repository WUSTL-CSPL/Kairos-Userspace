...
public:
    ...
    void ROSGrabImu(const sensor_msgs::ImuConstPtr &imu_msg);
...
public:
    ...
    void ROSGrabImageLeft(const sensor_msgs::ImageConstPtr& msg);
    void ROSGrabImageRight(const sensor_msgs::ImageConstPtr& msg);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_imu = node_handler.subscribe("/imu", 1000, &ImuGrabber::ROSGrabImu, &imugb); 
    ros::Subscriber sub_img_left = node_handler.subscribe("/camera/left/image_raw", 100, &ImageGrabber::ROSGrabImageLeft, &igb);
    ros::Subscriber sub_img_right = node_handler.subscribe("/camera/right/image_raw", 100, &ImageGrabber::ROSGrabImageRight, &igb);
...
void ImageGrabber::ROSGrabImageLeft(const sensor_msgs::ImageConstPtr &img_msg)
{
    mBufMutexLeft.lock();
    if (!imgLeftBuf.empty())
        imgLeftBuf.pop();
...
void ImageGrabber::ROSGrabImageRight(const sensor_msgs::ImageConstPtr &img_msg)
{
    mBufMutexRight.lock();
...
void ImageGrabber::SyncWithImu()
{
    ...
    while(1)
    {
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            CONSISTENCY_C(&mpImuGb->imuBuf, &imgRightBuf, 0.2, HandlingPolicy::ABORT);
...
void ImuGrabber::ROSGrabImu(const sensor_msgs::ImuConstPtr &imu_msg)
{
//     STABILITY(&imuBuf, 0.2, HandlingPolicy::ABORT);
