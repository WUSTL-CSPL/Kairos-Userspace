...
struct TimingInstrumenter : public ModulePass {
    ...
    bool runOnModule(Module &M) override {
        ...
        // Insert the initialization function at the end, we need the number of vertices
    ...
    void initialization(Module &M) {
        ...
        if (!index_file.is_open()) {
            std::cerr << "Failed to open file: " << _id_index_file_path << std::endl;
 ...
 //           std::cout << "- " << task.get<std::string>() << std::endl;
    ...
    bool instrumentGraphInitialization(Module &M) {
        ...
        for (Function &F : M) {
            ...
            if (F.getName().contains("main")) {
                ...
                FunctionCallee SetGraphSizeFunc = F.getParent()->getOrInsertFunction(
                    "setGraphSize",
                    FunctionType::get(Type::getVoidTy(Context), {Type::getInt32Ty(Context)}, false)
                );

                Value *graphSizeValue = ConstantInt::get(Type::getInt32Ty(Context), _timing_vertex_index + 1);
                Builder.CreateCall(SetGraphSizeFunc, {graphSizeValue});

                // Instrument to initialize the timing graph
                FunctionCallee init_timing_func = M.getOrInsertFunction("initTimingGraph", 
                ...
                for (std::string &name : _input_task_names) {
                    ...
                    FuncArgs.push_back(Type::getInt8PtrTy(Context)); // void* func (as int8*)

                    FunctionType *FuncType = FunctionType::get(Type::getVoidTy(Context), FuncArgs, false);

                    FunctionCallee addFunction = M.getOrInsertFunction("registerROSCallbackFunctionPtr", FuncType);
                    ...
                    for ( auto& pair : _input_function_to_id_map) {
                        if (pair.second == id) {
                            funcPtr = getFunctionPointer(M, pair.first->getName().str());
                        }
                    }
                    ...
                    // Value *id_value = ConstantInt::get(Type::getInt32Ty(Context), _input_function_name_to_id_map[name]);

                    Builder.CreateCall(addFunction, {Builder.getInt32(id), funcPtr});
    ...
    bool instrumentTimingDefUpdateByGraph(Module &M) {
        ...
        StructType *entryType = StructType::get(Ctx, {Type::getInt32Ty(Ctx), PointerType::get(Type::getVoidTy(Ctx), 0)});
 ...
 */
                    ...
                    for (auto &BB : F) {
                        ...
                        Value *id_value = ConstantInt::get(
                            Type::getInt32Ty(Ctx), _timing_vertex_index);
                    ...
                    Constant *id_constant = ConstantInt::get(
                        Type::getInt32Ty(Ctx), _timing_vertex_index);
                    Constant *func_ptr = ConstantExpr::getBitCast(
                        &F, PointerType::get(Type::getVoidTy(Ctx), 0));
                    idFuncEntries.push_back(ConstantStruct::get(
                        entryType, {id_constant, func_ptr}));
                    ...
                    _input_function_to_id_map.insert(
                        {&F, _timing_vertex_index});
                ...
                for (Instruction &I : BB) {
                    ...
                    if (isa<CallInst>(I) || isa<InvokeInst>(I)) {
                        ...
                        if (called_function &&
                            ...
                            is_annotated = 1;
                            parameters = 2;
                            if (called_function->getName().contains("consistency3")) {
                                parameters = 3;
                            }
                ...
                // *
                            if (auto *DVI = dyn_cast<DbgValueInst>(&I)) {
                                if (DVI->getValue()) {
                                    ...
                                    if (DIVar) {
                                        StringRef VarName = DIVar->getName();
                                        // Now VarName holds the original
                                        // variable name You can use VarName as
                                        // needed here
                                ...
                                errs() << "Cannot convert, Instruction is : " << I << "\n";
                            }
                */
                            //
                            ...
                            FunctionCallee checkTimingFunc =
                            ...
                            llvm::GlobalVariable *gloabl_should_abort_var = M.getNamedGlobal("shouldAbort");
                            ...
                            for (auto& pair : _function_to_input_dependency_map) {
                                if (function_name.contains(pair.first)) {
                                    // Get the vertex id
                                    for (std::string dep_function_name : pair.second){

                                        errs() << "The dependency function name is : " << dep_function_name << "\n";

                                        int dep_vertex_id = _input_function_name_to_id_map[dep_function_name];

                                        errs() << "The depenedency vertex id is: " << dep_vertex_id << "\n";

                                        // Store the dependency
                                        _vertex_dependency_map[_timing_vertex_index].push_back(dep_vertex_id);
                                    }
                                }
                            }
    ...
    Constant *getFunctionPointer(Module &M, const std::string &func_name) {
        ...
        return ConstantExpr::getBitCast(func,
