...
Set<SVF::VFGNode *> DepWalker::getVFGNodeUpperDefs(VFGNode *vfgnode, bool isRecord) {
    //   errs()<<"\nDaaaaDDDD \n";
...
SVFGNode *DepWalker::getSVFGNodeFromInst(Instruction *inst) {
    ...
    // Set<SVFGNode*> upperDefs;
    SymbolTableInfo::ValueToIDMapTy::const_iterator iter = _valSymMap.find(inst);
    Function *llvmfun = inst->getFunction();
    const SVFFunction *svfFun = _svfModule->getSVFFunction(llvmfun);
    SVFVar *node = _pag->getGNode(iter->second);

    if (iter == _valSymMap.end()) {
        // errs() << "NOTfound : " << *inst << "\n";
        return nullptr;
    }

    const SVFGNode *svfgnode = nullptr;
    if (_svfg->hasDefSVFGNode(node)) {
        svfgnode = _svfg->getDefSVFGNode(node);
        //  errs()<<" here is vfgnode "<<svfgnode->toString()<<"\n";
    } else {
        //   errs()<<" NOOOOO vfgnode "<<node->toString()<<"\n";

        const IRGraph::SVFStmtSet &svfStmtSet = _pag->getValueEdges(inst);
        //  errs()<<" SSSNOOOode "<<"\n";
        ...
        for (auto it : svfStmtSet) {
            ...
            if (const ReturnInst *retInst =
                ...
                const PAGNode *fun_return = _pag->getFunRet(svfFun);
                svfgnode = _svfg->getFormalRetVFGNode(fun_return);
                //  errs()<<" Return vfgnode "<<svfgnode->toString()<<"\n";
            } else {
                // errs() << "\n others " << it->toString();
                if (_svfg->hasStmtVFGNode(it)) {
                    svfgnode = _svfg->getStmtVFGNode(it);
                }
...
Set<SVFFunction *> DepWalker::getDistinguishFunction(
    ...
    for (auto item : llvmfset) {  // first initialize the reacahble func map
        ...
        SVFFunction *svffun = (SVFFunction *)_svfModule->getSVFFunction(fun);
        fset.insert(svffun);
        // errs()<<"cdssd\n";
        Set<SVFFunction *> tmp = _ptacallgraph->getReachableFunctions(svffun);
        // errs()<<"cdwc\n";
    ...
    for (auto sfun : funs2analysis) {  // initialize the reachstate of each fun,
        ...
        for (PTACallGraph::CallGraphEdgeConstIter
                 it = _ptacallgraph->getCallGraphNode(sfun)->OutEdgeBegin(),
                 eit = _ptacallgraph->getCallGraphNode(sfun)->OutEdgeEnd();
             it != eit; ++it) {
        ...
        for (auto eqedge : equivalanceedges) {
            ...
            for (auto edge : eqedge.second) {
                ...
                for (auto tmp : idcs) {
                    ...
                    _ptacallgraph->getCallees(tmp, callees);
                    int notnull = 0;
